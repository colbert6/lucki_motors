...
class Pdf_comprobantes extends TCPDF
{
    ...
    public function comprobante_data_b($n_col , $data ) {
        $this->set_formato($this->format);               
        ...
        if( $this->format == 'A4'){
            ...
            $this->comprobante_data['w'] = $this->max_width;
            $this->comprobante_data['max_col'] = $n_col;
        ...
        foreach ($data as $key => $value) {
            ...
            if($cd['max_col'] != 1){
                $text.="<th colspan='".$value[1]."' > <strong>".$key."</strong> : ".$value[0]."</th>" ;
                $colspan += $value[1];
                if($colspan % $cd['max_col'] == 0  ) {
                    $text.= '</tr><tr>';
                }
            }else if($cd['max_col'] ==1){
                $i_row++;
                $text.="<th> ".$key." : ".$value[0]."</th>" ;
                if( $i_row < $max_rows ) {
                    $text.= '</tr><tr>';
